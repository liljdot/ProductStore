        ...
        <>
            <div className="dropdown dropdown-end">
                ...
                <div tabIndex={0} className="dropdown-content mt-2 p-1 shadow-2xl bg-base-200 backdrop-blur-lg rounded-2xl w-56 border border-base-content/10">
                    {THEMES.map(theme => (
                        <button key={theme.name} onClick={() => setTheme(theme.name)} className={` w-full px-4 py-3 rounded-xl flex items-center gap-3 transition-colors ${currentTheme == theme.name ? "bg-primary/10 text-primary" : "hover:bg-base-content/5"}`}>
